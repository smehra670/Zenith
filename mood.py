...
daily = Agent(
    ...
    instructions=[
        ...
        "The task should be as difficult as doing 20 push-ups.",
        "It should be activities that can be done at home",
...
@tool
def workout(difficulty: str, option: str, time: int):
    """Give a workout plan based on mood, difficulty, options and time."""
    return {
...
diet = Agent(
    ...
    instructions=[
        ...
        "Highlight any key tips (e.g., hydration, meal prep).",
        "If cutting → aim for high protein, moderate carbs, low-moderate fats.",
        "If bulking → aim for high protein, high carbs, moderate fats."
...
work = Agent(
    ...
    tools=[workout],
    instructions=[
        "Always CALL the workout tool using the given mood/difficulty/option/time.",
...
posture_agent = Agent(
    ...
    model=Groq(id=POSTURE_MODEL_ID),
    instructions=[
        "You are an expert fitness coach analyzing workout form from photos.",
        "You will receive 1-3 workout photos and may receive the intended exercise name.",
        "Analyze the image carefully and provide feedback in VALID JSON format ONLY.",
        "",
        ...
        "",
        "Guidelines:",
        "- If user provides exercise name, assume that movement unless images clearly show otherwise",
...
def generate_correct_form_image(exercise_name, corrections_data):
    ...
    try:
        ...
        prompt_parts = [
            ...
            "Th psoture and form should be perfect and not the image the user gives as thats most likely incoorrect.",
        ...
        form_guidance = {
            ...
            'push-up': "straight plank position, elbows at 45-degree angle, controlled movement",
